...
{
    float* ray;
    cudaMalloc((void**)&ray, sizeof(float)* ray_nums);//(void**)&  其中(void **)是一个强制类型转换的。也就是把一个东西变为指向指针的指针    
    fetch_origx<<<1,1>>>(ray,ray_addr);
    float* ray_out;
    ray_out = (float*)malloc(sizeof(float)*ray_nums);
    cudaMemcpy(ray_out, ray, sizeof(float)*ray_nums, cudaMemcpyDeviceToHost);
    cudaFree(ray);
    return ray_out;
}
